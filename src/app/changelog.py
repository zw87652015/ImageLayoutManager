...
CHANGELOG = [
    ("3.4.2", "2026-09-21", [
        ...
        {"en": "Existing projects keep their original text sizing and layout in clearly labelled legacy mode; their size values are not automatically converted.",
         "zh": "旧工程保留原有字号与布局，并明确显示旧版单位；不会自动转换原数值。"},
        {"en": "Projects saved by this version use an updated file format; keep an original copy if you need to return to an older ILM version.",
         "zh": "本版本保存的工程采用更新后的文件格式；若需要返回旧版 ILM，请保留原工程副本。"},
        {"en": "Improved fractional SVG text sizing and separately positioned text lines in new projects, without changing source files.",
         "zh": "改进新工程中 SVG 小数字号和独立定位文字行的渲染，不修改源文件。"},
        {"en": "Label Cell Properties now separates This label from All labels: ordinary style and position edits affect only the selected label, with explicit Apply Style to All and Apply Position to All buttons.",
         "zh": "标注单元格属性区分“当前标注”和“全部标注”：普通样式与位置编辑只影响所选标注，批量修改需使用“将样式应用到全部”或“将位置应用到全部”。"},
        {"en": "Replaced the label alignment dropdown with a visual picker: left/centre/right above or below images, and top/middle/bottom beside images; alignment stays on the chosen edge after quarter-turn rotation.",
         "zh": "用可视化选择器替代标注对齐下拉框：图像上下方可选左、中、右，左右侧可选上、中、下；旋转 90 度后仍保持所选边缘对齐。"},
        {"en": "Removed duplicate Selected Label controls and the custom-style notice from strip-label settings; Reset to Tier Style remains available.",
         "zh": "独立标注设置不再重复显示“当前标注”分区和自定义样式提示，仍可使用“恢复为层级默认样式”。"},
        {"en": "The Auto Above-Cell Labels button's dropdown now offers only above, below, left and right placements; in-cell labels retain their separate button.",
         "zh": "“自动图外标注”按钮的下拉菜单只保留上、下、左、右四种位置，图内标注仍使用独立按钮。"},
        {"en": "SVG and raster text tools now share the name Match Text Size; raster review rows use cleaner status text without the leading icons.",
         "zh": "SVG 与位图文字工具统一命名为“匹配文字大小”；位图审查列表移除状态文字前的图标，显示更简洁。"},
        {"en": "Reveal image sources in the system file explorer from Layers or canvas context menus, with separate targets for the main cell image and each inset.",
         "zh": "可从图层或画布右键菜单在系统文件资源管理器中显示图像源文件，主图与每张插图分别定位各自来源。"},
        {"en": "Fixed selection getting stuck on an inset when returning to its parent cell, including the Layers highlight and Inspector contents.",
         "zh": "修复从插图切回主单元格时选择仍停留在插图的问题，图层高亮与检查器内容现在会正确切换。"},
        {"en": "Fixed a crash when changing the numbering scheme, unintended style locking when switching label tiers, and position drift when moving point-sized labels with backgrounds.",
         "zh": "修复修改编号方案时的崩溃、切换标注样式层级时意外锁定样式，以及移动带背景的磅值标注时的位置偏移。"},
        {"en": "Reworked English and Chinese tutorials and help to explain goals before actions; the first lesson now introduces rows and cells before building its three-image example.",
         "zh": "重写中英文教程与帮助，先说明目标再引导操作；第一课先介绍行与单元格，再搭建三张示例图的布局。"},
        {"en": "Corrected guidance for inset drop zones, export regions, plot alignment and text units, and clarified the difference between saving a project, exporting and previewing.",
         "zh": "修正插图投放区、导出区域、绘图区对齐和文字单位的说明，并明确保存工程、导出成图与预览之间的区别。"},
    ]),
    ("3.4.1", "2026-09-18", [
