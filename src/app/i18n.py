...
_T: dict[str, dict[str, str]] = {
    ...
    "plot_custom_note": {"en": "Equal row numbers share a baseline. Suggested rows follow current vertical positions; edit the visible row numbers to regroup.", "zh": "相同分行编号共用底部基线。建议分行依据当前垂直位置；可编辑表格中的编号重新分组。"},
    "plot_single_note": {"en": "All checked panels share one bottom baseline; use separate rows for multi-row figures.", "zh": "所有勾选面板共用一条底部基线；多行图请使用分行对齐。"},
    ...
    "plot_issue_invalid_rows": {"en": "Assign a row number (0 or greater) to every checked panel", "zh": "请为每个勾选面板设置非负整数分行编号"},
    ...
    "plot_guide_rows": {"en": "Set a row number for every checked image.", "zh": "请为每张勾选图像设置分行编号。"},
    ...
    "plot_guide_issue_invalid_rows": {"en": "Review row numbers for every checked image.", "zh": "请检查每张勾选图像的分行编号。"},
    # ── Menus ────────────────────────────────────────────────────────
